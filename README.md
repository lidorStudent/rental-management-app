...
| Tenant | `yonatan.azoulay@example.co.il` | An active tenancy of Noa's carrying a part payment against the current month: some of the rent has arrived in cash, the rest has not. The month reads **Part paid** while its due day is still ahead and **Overdue** once that day has passed, because past due outranks part paid — a month that needs chasing should not be filed as merely incomplete. The remainder is worth signing in as both parties to see, because the two sides disagree until the due day and agree after it. The landlord's schedule row on that tenancy carries the remainder in its Outstanding column from the moment the payment is recorded. The tenant's Outstanding figure is "everything charged so far, less what your landlord has recorded", and a month is not charged until its due day passes, so before that day their portal reads nothing owed and after it the two figures match |
