...
of the fifty-seven component files are client components, and every one of them is there for
...
| Suite | Proves |
| --- | --- |
| `npm test`, 361 tests | The rules are right at their boundaries, and the forms behave, including for a screen reader |
| `npm run test:db`, 146 tests | The policies and constraints hold against a real Postgres, as real signed-in users |
| `npm run test:e2e`, 28 of 35 tests | The whole processes work in a browser, including the refusals |
...
everything as props. Twenty-five of fifty-seven component files are client components, and none of
...
146 tests attack the database directly, and they are the evidence that the claim is true.
