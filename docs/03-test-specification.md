...
| 2026-09-02 | Claude | Chromium 141, from production | **Pass, after a fix** | Re-run because the mark printed as a solid black rectangle on both routes. Cause: a CSS mask is a paint-time effect, and a print pipeline that flattens the page without applying it paints the background across the whole box, which `print-color-adjust: exact` then guarantees reaches the paper. The statement now uses a plain `<img>`, so the PDF carries the 150 by 256 artwork with its alpha channel instead of a filled rectangle. Printed both the landlord and the tenant route from production: two-page A4, the mark reads as the mark at 1.8 percent ink inside its box rather than 100, no navigation, no buttons, no range form, margins even and nothing clipped. The payments table broke across the page with its header repeated and no row split; the summary and footer stayed whole on page two. The footer's "settle" and "differ" render correctly on the page; where they extract as "se*le" and "di+er" that is the producing application's font subsetting, not the document |
| 2026-09-03 | Claude | Chromium 141, from production | **Pass** | Re-run after the component extractions and the deletion of `public/logo.png`, both of which could have reached this page: the statement draws the mark from `public/logo-mark.png`, which is a different file and still served. Both routes printed, landlord and tenant, and the two came out **pixel-identical** page for page, which is right - they render the same component for the same tenancy. Two pages of A4, 92 KB. The mark prints as the mark: rasterised at 300 dpi its box measures 3.9 percent ink, where a black box would be near 100. No navigation, no sign-out, no print button, no range form and no back link, in the image or in the text layer - searching the extracted text for nine chrome strings returns zero on both routes. The payments table breaks across the page with its column header repeated and no row split; the summary block and the footer stay whole on page two. Margins even, nothing clipped |
| 2026-09-06 | Claude | Chromium 141, from production | **Pass, with one departure** | Re-run because `globals.css` changed on 2026-09-05 (554f441), so the file this case covers is not the file it was last run against. That commit removes one line, `@import "tw-animate-css"`, and does not touch the single `@media print` block. Both routes printed to A4 from production. **The mark renders as the mark on both**, an outlined building glyph, not the solid black rectangle of the 2026-09-02 defect. In print media the navigation, every button, the back link and the range form are all hidden on both routes, and the statement heading stays. Landlord route, the ended tenancy on Flat 3 over its whole term: two A4 pages, thirteen month rows, all sections present, totals ₪80,600.00 charged and received and ₪0.00 outstanding. Tenant route, Maya's nine months: two A4 pages, and the payments table splits across the boundary with its header repeated, no row broken. Margins even, nothing clipped at the right edge. **The departure:** on the landlord route the "Payments received" heading is the last line of page 1 and its table begins on page 2, which step 5 says to check for. There is no `break-after` or `break-inside` rule anywhere in `globals.css`, so widow and orphan control is the browser's default and where the break falls moves with the length of the statement; the seed is date-relative, so this run met it and the earlier ones did not. Not a regression, and not fixed here |
...
| --- | --- | --- | --- | --- |
| 2026-09-03 | The developer | Desktop and mobile | **Pass** | Run against the current build on production, after the component extractions. Checked the four screens where the shared `DetailRow` replaced the four copies: the landlord lease detail, the landlord maintenance detail, the tenant lease page and the tenant portal. All four render correctly. Labels and values aligned, row heights consistent, money right-aligned and lining up in its column, and the linked rows - Unit on the lease detail, Open the lease on the maintenance detail - still render and behave as links. Nothing shifted |
| 2026-09-06 | The developer | Desktop and mobile | **Pass** | Re-run after the landlord loading skeleton was rewritten (299fea8, 2f9ae97) and `tw-animate-css` was removed from `globals.css` (554f441). Judged by eye; no layout problem found |
...
| --- | --- | --- | --- |
| 2026-09-02 | The developer | **Pass** | Run end to end against production on the current interface. A fresh temporary password was issued from the landlord side, and signing in as that tenant held them on the change-password page: navigating elsewhere returned them to it. Setting a new password landed them in the tenant portal. The page carries the logo and the Rentbook name |
| 2026-09-06 | Claude | **Pass** | Re-run against the test project on the current build, because the flow changed three times since the previous run: the forced-change guard moved into the role guards (9c2df16), attaching the account became a compare-and-set (063170e), and the flag is now written before the password (01c4980). A landlord, building, unit and tenancy were built through the admin API and removed afterwards. **The landlord side:** the panel reads "Give this password to your tenant now", then "It is shown once and cannot be shown again. Nothing stores it, not even this application, so if it is lost you will have to issue a new one", a fourteen-character password, a Copy control and "I have given it to them". After a reload the password is nowhere in the page and the panel reads "Tenant / MAN-03 Tenant / Signs in with / <address>". **The tenant side, in a fresh context:** signing in with the temporary password lands on `/change-password`, headed "Set your own password" with "Your landlord created this account with a temporary password. Choose your own before continuing", and the Current password field explains it is "the one you signed in with". `/tenant`, `/tenant/payments`, `/tenant/maintenance/new` and `/landlord` all return to `/change-password`. Setting a new password lands on `/tenant`, headed "Your tenancy". The temporary password is then refused at `/login` with "That email address and password do not match an account", and the new one signs in |
...
2. Check the focus outline is visible at every step. Tab order follows visual order at desktop
   width. At 768 and 390 it does not, and the departure is known: the header's navigation is second
   in the DOM but wraps onto its own second row, so the keyboard reaches it before the sign-out
   button drawn above it. Confirm that this is the only departure, and that every control is still
   reachable in a sequence that makes sense. Do not report it as a new fault; it is recorded as a
   known limitation in the technical plan.
...
| --- | --- | --- | --- | --- |
| 2026-09-02 | The developer | Keyboard only | **Pass** | Tabbed through the landlord header on the current interface. The logo and the wordmark are one focus stop, not two. Every stop shows a visible focus ring. Enter on the lockup goes to the dashboard. Checked in Safari with "Press Tab to highlight each item on a webpage" enabled, because Safari does not tab to links without it |
...
| 2026-08-27 | Claude, by script | Keyboard only | **Pass** | Re-run across both portals after the theme and control changes, by tabbing through 19 screens and measuring every stop 500ms after the key, so the button's 150ms ring transition had settled. 201 focus stops: 198 carry a visible indicator, and 0 depart from the visual order. The other 3 are the calendar button Chromium puts inside `<input type="date">`, which is browser shadow DOM: `:focus-visible` stops matching on the host input, the browser draws its own indication, and no application style reaches it. The date segments themselves each carry the ring. **A defect was reported against the primary button and there is none**: a first measurement read `box-shadow` at the instant of focus, before the button's `transition-all` had drawn the ring. Measured after it settles, the button carries the same 3px accent ring as an input. The 2026-08-26 note above was right and is left standing. **Scope, added 2026-09-02:** this run recorded no departures, which places it at desktop width. The header departure now recorded in the technical plan appears on every screen below the `lg` breakpoint, so a run that met it at 768 or 390 could not have returned zero. The figure stands for the width it covered and is not a claim about narrow ones |
| 2026-09-06 | Claude, by script | Keyboard only | **Pass** | Re-run the way the 2026-08-27 row was, after 21 commits touching 41 files under `src/components` and `globals.css`. Fifteen screens across both portals and the two public pages, at 1440, 768 and 390, measuring every stop 500ms after the key so the ring transition had settled. **451 focus stops in all: 150 at 1440, 150 at 768, 151 at 390. Every one carries a visible indicator; none does not.** Departures from visual order: **0 at 1440, 13 at 768, 13 at 390** — one on each of the thirteen signed-in screens and none on `/login` or `/register`. Every one is the known header limitation and no other: at 768 the sequence runs wordmark (y=12), the five navigation links (y=52), then Sign out (y=14), which is drawn above the row the keyboard has just left. That is the departure recorded in the technical plan at section 19, and step 2 says not to report it as a new fault. Every control remains reachable in a sequence that makes sense. **One difference in method from the 2026-08-27 run, which found 3 stops without a ring:** this run reads `document.activeElement`, which resolves shadow content to its host, so Chromium's own calendar button inside `<input type="date">` is not counted as a separate stop. Same interface, different counting; the 3 are not fixed, they are not visible to this measurement. The VoiceOver rows above were not re-run |
...
| 2026-08-30 | Claude | https://rental-management-app-wine.vercel.app | **Pass** | Re-run after the rent-overview fix and after one hand-made tenancy was deleted from the deployed project, restoring it to exactly what the seed writes: 3 properties, 7 units, 5 leases, 29 payments, 5 requests, 6 profiles. All seven passed in 16.2s. The dashboard reads occupancy **2 of 5** as this document and the README describe. Portfolio outstanding reads **₪16,300.00**, not the ₪13,000.00 recorded on 2026-08-26: Yonatan's rent fell due on the 28th in the two days between, so his ₪3,300.00 now counts as arrears. Both figures were correct on the day they were taken, which is the date dependence the demo-account descriptions were corrected to state. Read only |
| 2026-09-03 | Claude | https://rental-management-app-wine.vercel.app | **Pass** | Re-run against the current build, five days and four phases after the previous run: the rename to Rentbook, the logo in both headers, the component extractions and the deletion of `public/logo.png` had all landed since. All seven checks passed in 17.8s. Read only. The health endpoint answers 200 with the database reachable. `noa.bendavid@example.co.il` lands on `/landlord` showing ₪6,500.00 outstanding across every tenancy, three problems reported and not resolved, occupancy 2 of 5, and ₪0.00 collected this month. `maya.levi@example.co.il` lands on `/tenant` showing Flat 1, Rothschild Boulevard 12, Tel Aviv-Yafo, with ₪6,500.00 due on 2026-09-10 and one month past its due date. Those figures move with the date, because they are derived from the ledger and today rather than stored |
| 2026-09-06 | Claude | https://rental-management-app-wine.vercel.app | **Pass** | Re-run against the current build, after the document corrections of 2026-09-06. `x-vercel-id: fra1::fra1`, health `{"status":"ok","database":"reachable"}`. The automated companion was run with `PLAYWRIGHT_BASE_URL` set and all seven passed in 18.9s, SEC-01 and SEC-02 among them. By hand: the root redirects to `/login`; the landlord dashboard reads Outstanding ₪6,500.00, Open problems 3, Occupancy 2 of 5; the Flat 1 lease, its statement (Total charged ₪58,500.00) and the maintenance list all load; the tenant portal opens on "Your tenancy" with this month ₪6,500.00 Due 2026-09-10. **Rent collected this month reads ₪0.00**, with its own line "Nothing recorded yet this month": every seeded receipt predates September, which the rent overview confirms (Maya received ₪45,500.00 of ₪52,000.00 charged, oldest unpaid 2026-08-10). Correct for a date-relative seed that has not been re-run this month, not an empty page |
...
the assertions exist; the citations do not". That was wrong on both counts. All nine are E-level
cases about the message a refusal produces, and the schema tests prove nothing about messages; and
of the nine, only three had an assertion behind them at all. Three were cited, six were written.

| Case | Where it lives now | Watched failing against |
| --- | --- | --- |
| INV-08 | `e2e/negativePaths.spec.ts` | A sign-in that looked the address up and answered "No account exists for that address." The test read `expected "That email address and password do not match an account." received "No account exists for that address."` |
| INV-09 | `e2e/negativePaths.spec.ts` | The same break. The two cases share one test, because the property is that the two answers are indistinguishable and neither half means anything alone |
| INV-34 | `tests/serverActions.test.ts` | The later-end-date branch removed from `refuseIfNewEndDateIsNotEarlier`: the action returned success and extended the tenancy |
| INV-35 | `tests/serverActions.test.ts` | The ends-before-it-started branch removed from the same function |
| INV-40 | `tests/serverActions.test.ts` | The month-within-tenancy guard neutered in `rentPaymentActions`: the payment was accepted against a month before the tenancy began |
| INV-49 | `tests/serverActions.test.ts` | The transition check neutered in `maintenanceRequestActions`: a request in progress was moved back to submitted |

**A correction about INV-49.** An earlier note here said the sentence this case quotes does not exist
anywhere in the code. It does. The message is built rather than written down -
`` `A request that is ${readable(status)} can only become ${allowedNextStatuses(status).map(readable).join(" or ")}.` `` -
and `readable` is `status.replace("_", " ")`, so a request in progress produces exactly "A request
...
authorisation and never the rule behind it. The suite had exactly such a test for `endLease` and it
passed, which is what made the gap look covered. The new cases act as the landlord who owns the row.
