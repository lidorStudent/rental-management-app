 ...
 *
 * The mark here is a crop of the artwork, not the whole of it. The whole mark is portrait, so a
 * header that is 24 pixels tall leaves it 14 pixels of width, and its busiest scanline crosses five
 * strokes in those 14 pixels; they merge into a smudge. The upper form on its own is near-square, so
 * the same 24 pixels of height buy 23 of width for three strokes. Carrying the whole mark here
 * instead would mean a 40 pixel header row, which costs 12 pixels of height at 390 and still reads
  ...
  return (
    <Link href={href} className="flex items-center gap-2 text-sm font-semibold tracking-tight">
      <span
        aria-hidden="true"
        style={HEADER_MASK}
        className="bg-foreground block h-6 w-6 flex-none"
      />
      Rental Management
    </Link>
  );
